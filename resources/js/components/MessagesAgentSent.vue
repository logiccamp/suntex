                            ...
                            <tbody>
                                <tr v-for="(message, index) in messages.slice().reverse()" :key="index" :class="message.status" style="margin: 5px 0 !important;">
                                    ...
                                    <td class="mailbox-name">
                                        <a :href="'/agent/notification/read/'+message.mailid">{{message.mailid.substr(0, 5)}}</a>
                                    </td>
                                    <td class="mailbox-subject">
                                        <a style="text-decoration: none;" :href="'/agent/notification/read/'+message.mailid">
                                            <b style="text-transform: capitalize">{{message.to.firstname}}</b> - {{message.subject}}
                                        </a>
                                    </td>
...
export default {
    ...
    methods: {
        fetchMessages(){
            axios.get(`/api/get-emails/sent/${this.id}`)
            .then((response)=>{
